...
CODEBUDDY_POLL_TIMEOUT_SECONDS = float(
    os.getenv("BATCHER_CODEBUDDY_POLL_TIMEOUT_SECONDS", "30")
)
...
async def _save_cookies_to_file(page: Any, email: str) -> bool:
    ...
    try:
        ...
        cookie_data = {
            "email": email,
            "saved_at": time.time(),
            "expires_at": time.time() + 86400,  # 24 hours (server validates the real session)
            "cookies": cookies,
        }
...
class CodeBuddyProviderAdapter(ProviderAdapter):
    ...
    async def bootstrap_session(self, account: NormalizedAccount) -> Any:
        ...
        try:
            ...
            camoufox_kwargs = build_camoufox_kwargs(
                extra={"disable_coop": True},  # CodeBuddy needs COOP disabled for Turnstile
            )
    ...
    async def fetch_quota(
        ...
        _codebuddy_auth_debug("VIP: credit fetch failed (API + browser), returning fixed 250 credits fallback")
        return {
